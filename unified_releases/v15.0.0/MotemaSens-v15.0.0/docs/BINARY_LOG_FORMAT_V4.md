...
Format v4 replaces the fixed ECG-rate combined record with independently timestamped acquisition chunks. It preserves configurable ECG (250/500/1000/2000 Hz), microphone (500/1000/2000 Hz) and IMU (31.25/62.5/125/250 Hz) streams without copying a latest value into another stream's row. The selected rates are encoded in the reserved header rate metadata and are decoded by the Python, Flutter and MATLAB readers.
...
The firmware closes the file and then re-opens it on Core 1 to compare the stored trailer and recompute the payload CRC32 with a bounded sequential read. The verifier yields periodically so a long recording cannot starve the Core 1 idle task. Chunk type, payload length, final chunk count and trailer consistency are validated by the Python, Flutter and MATLAB readers when the file is downloaded. Only a matching on-device trailer/CRC readback is reported as a verified recording through local Wi-Fi status, cloud status and the mobile app. A failed SD write or failed reread is reported as a failed stop; it must not be described as a successful recording.

The active filename ends in `.bin.part`. It is renamed to `.bin` only after the clean-stop readback succeeds. On boot, any abandoned `.bin.part` is preserved as `.incomplete.bin` for recovery and is never silently promoted to a complete file.
